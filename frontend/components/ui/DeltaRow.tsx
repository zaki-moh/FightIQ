...
const DeltaRow = ({
  ...
  const cap = VISUAL_CAP[label.toLowerCase()] ?? 1
  const widthPct = Math.min((Math.abs(delta ?? 0) / cap) * 100, 100)
  ...
  if (isContext) {
    return (
      <div className="grid grid-cols-[75px_1fr_100px] items-center gap-3 w-full">
        ...
        <span className="text-[11px] text-white/45 italic tracking-wide">{context}</span>
      </div>
    )
  }
  ...
  return (
    <div className="grid grid-cols-[75px_1fr_100px] items-center gap-3 w-full">
      ...
      <div className="relative h-2.5 rounded-full bg-white/5 overflow-hidden">
        <div
          className={`absolute inset-y-0 ${barAlignment} flex`}
          style={{ width: '100%' }}
        >
          <div
      ...
      <span className="text-xs tracking-wide text-white/80 tabular-nums">
        ...
        {unit === '%' && <span className="text-white/40 ml-1">pp</span>}
      </span>
    </div>
  )
}
